...
public class MetadataResolverServiceTest {
    ...
    public void shouldProvideCorrectMetadataResolverForEntityId() {
        ...
        when(metadataResolverFactory.create(any(Client.class), eq(URI.create(AN_ENTITY_ID)), any(List.class), eq(MIN_REFRESH_DELAY_MS), eq(MAX_REFRESH_DELAY_MS))).thenReturn(metadataResolver1);
